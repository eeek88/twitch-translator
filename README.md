...
note once it's ready (a beat after the line appears — it's generated by a
separate small model). This runs entirely on CPU, off the GPU pipeline
feeding your captions, so it never adds latency or competes for VRAM with
Whisper/NLLB. Disable it with `--no-context-helper` if you'd rather not pay
the CPU cost at all.
